<footer class="footer-section">
    <div class="section-gap double-gap">
        <div class="container">
            <div class="row">
                <div class="col-md-7">
                    <div class="quick-links">
                        <h3 class="footer-heading">Quick Links</h3>
                        <ul class="d-flex align-items-center">
                            <li><a href="#">About Us </a></li>
                            <li><a href="#">UCEED Score Accepted</a> </li>
                            <li><a href="#">Admission Process</a></li>
                            <li><a href="#">Recruiters</a></li>
                        </ul>
                    </div>
                </div>
                <div class="col-md-5 mt-md-0 mt-4">
                    <div class="address">
                        <h3 class="footer-heading">Address</h3>
                        <div class="d-flex align-items-baseline main-address">
                            <span><i class="n-icon" data-icon="i-location"></i></span>
                            <div class="adress-title">
                                <h4 class="sub-footer-title">School of Design</h4>
                                <p>SVKM's NMIMS (Deemed to be University) 6th floor, UPG
                                    Building, V. L. Mehta Road, Vile Parle - West,Mumbai, Maharashtra, India. Pin Code –
                                    400056</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row mt-4 pt-md-2 pt-4">
                <div class="col-md-7 order-md-1 order-2 mt-md-0 mt-4 pt-md-0 pt-4">
                    <div class="quick-links social-icon">
                        <h3 class="footer-heading">Follow us</h3>
                        <ul class="d-flex align-items-center">
                            <li><a href="#"><i class="n-icon" data-icon="i-facebook"></i> </a></li>
                            <li><a href="#"><i class="n-icon" data-icon="i-twitter"></i> </a></li>
                            <li><a href="#"><i class="n-icon" data-icon="i-instagram"></i> </a></li>
                            <li><a href="#"><i class="n-icon" data-icon="i-linkdin"></i> </a></li>

                        </ul>
                    </div>
                </div>
                <div class="col-md-5 order-md-2 order-1 ">
                    <div class="address">
                        <h3 class="footer-heading">Contact info</h3>
                        <h4 class="sub-footer-title contact-main">Ms. Blanche Dias / Ms. Vidya Thorat</h4>
                        <div class="d-flex align-items-baseline main-address">
                            <span><i class="n-icon" data-icon="i-phone"></i></span>
                            <p>
                                <a href="tel:022 4233 5190">022 4233 5190</a> /
                                <a href="tel:022 4233 5190">4235 5821</a>
                            </p>
                        </div>
                        <div class="d-flex align-items-baseline main-address">
                            <span><i class="n-icon" data-icon="i-email"></i></span>
                            <p>
                                <a href="mailto:blanche.dias@nmims.edu">blanche.dias@nmims.edu</a> / 
                                <a href="mailto:vidya.thorat@nmims.edu">vidya.thorat@nmims.edu</a>
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="container">
        <div class="col-12">
            <div class="copyright text-center">
                <p>Copyright 2025. SVKM'S NMIMS. All rights reserved.</p>
            </div>
        </div>
    </div>
</footer>